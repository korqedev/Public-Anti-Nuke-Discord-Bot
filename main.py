...
LOG_CHANNEL_ID = int(os.getenv("LOG_CHANNEL_ID", "1517946086638096424"))
QUARANTINE_ROLE_ID = int(os.getenv("QUARANTINE_ROLE_ID", "1518726374612009091"))
STAFF_ROLE_ID = int(os.getenv("STAFF_ROLE_ID", "1517815722762305570"))
TRUSTED_MOD_BOT_ID = int(os.getenv("TRUSTED_MOD_BOT_ID", "1519361811244122113"))
...
async def notify_owner(guild, title, description):
    ...
    if not user:
        try:
            ...
            return
    try:
        await user.send(embed=discord.Embed(title=title, description=description, color=discord.Color.red(), timestamp=discord.utils.utcnow()))
    except discord.HTTPException:
        pass
...
async def dm_quarantined_user(member, action, target, details):
    embed = discord.Embed(
        ...
        description=(
            "NICE TRY TRYING TO NUKE US DICKHEAD\n\n"
...
async def quarantine(guild, actor, action, target, details):
    if actor is None:
        ...
        return False
    if is_owner_exempt(actor) or actor.id == TRUSTED_MOD_BOT_ID or (bot.user and actor.id == bot.user.id):
        return False
...
def build_overwrites_from_backup(guild: discord.Guild, data: dict):
    overwrites = {}
    for target_id, item in data.items():
        target = guild.get_role(int(target_id)) if item.get("type") == "role" else guild.get_member(int(target_id))
        if target is None:
            continue
        ...
        if category is None:
            try:
                category = await guild.create_category(
                    channel_data["name"],
                    overwrites=build_overwrites_from_backup(guild, channel_data.get("overwrites", {})),
                    reason="Rise Tweaks Security backup recovery",
                )
...
async def emergency_lock_guild(guild: discord.Guild, reason: str):
...
async def activate_join_raid_mode(guild: discord.Guild, count: int, seconds: int):
    ...
    changed = await emergency_lock_guild(guild, "Rise Tweaks Security automatic join-raid lockdown")
    text = (
...
async def on_guild_role_create(role):
    ...
    if (triggered or dangerous_role(role)) and result.actor and result.actor.id != OWNER_ID:
        try:
...
async def on_guild_role_update(before, after):
    ...
    if added and result.actor and not is_owner_exempt(result.actor):
        triggered = True
        await quarantine(after.guild, result.actor, "role_update", after.mention, f"Added: {', '.join(added)}")
    if triggered and after < after.guild.me.top_role:
        try:
...
async def unlockdown(interaction):
    backup = load_json(LOCKDOWN_FILE, {})
    if not backup:
        return await interaction.response.send_message("❌ No backup found.", ephemeral=True)
    await interaction.response.defer(ephemeral=True)
    restored = failed = 0
    for cid, values in backup.items():
        channel = interaction.guild.get_channel(int(cid))
        if not channel:
            continue
        ow = channel.overwrites_for(interaction.guild.default_role)
        ow.send_messages = values.get("send_messages")
        ow.add_reactions = values.get("add_reactions")
        ow.connect = values.get("connect")
        ow.speak = values.get("speak")
        try:
            await channel.set_permissions(interaction.guild.default_role, overwrite=ow, reason="Rise Tweaks Security unlockdown")
            restored += 1
        except discord.HTTPException:
            failed += 1
